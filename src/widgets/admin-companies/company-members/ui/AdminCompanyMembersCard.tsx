...
const roleOptions: CompanyMemberRole[] = ["OWNER", "ADMIN", "LOGIST", "MANAGER", "VIEWER"];
const statusOptions: CompanyMemberStatus[] = ["INVITED", "ACTIVE", "BLOCKED", "REMOVED"];
                ...
                <Stack spacing={2}>
                    ...
                    >
                        <Stack spacing={0.5}>
                            ...
                            <Typography variant="body2" color="text.secondary">
                                Здесь администратор может менять роли, статусы, назначать нового владельца и удалять участников.
                            </Typography>
                        </Stack>
                    ...
                    {isLoading ? (
                        ...
                        <Typography color="text.secondary">
                            {showHistory ? "История участников пуста." : "Участники не найдены."}
                        </Typography>
                    ) : (
...
function AdminCompanyMemberRow(props: {
    ...
    const { companyId, member, currentOwnerUserId, onUpdated, onRemoved, onOwnerChanged } = props;
    ...
    const title = fullName || member.user?.email || member.user?.phone || member.user_id;
    ...
    const handleSave = async () => {
        try {
            ...
            const updated = await adminCompaniesApi.updateMember(companyId, member.id, {
                role,
                status,
        ...
        >
            ...
            >
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap">
                    ...
                    {isCurrentOwner ? <Chip label="Текущий владелец" color="primary" size="small" /> : null}
                    {member.is_default ? <Chip label="По умолчанию" color="success" size="small" /> : null}
                </Stack>
            </Stack>
