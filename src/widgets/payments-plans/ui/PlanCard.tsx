...
export const PlanCard: React.FC<Props> = ({
  ...
  const renderBillingPeriod = React.useCallback(
    (plan: TariffPlan) => {
  ...
  const discountInfo = useMemo(() => {
    ...
    const hasExpired =
      ...
      new Date(user.discount_expires_at).getTime() <= Date.now();
  ...
  const features = useMemo(
    () => [
      {
        label: t("paymentsNew.orderDetails", "Order details access"),
      ...
      {
        label: t("paymentsNew.autoBump", "Can auto bump orders"),
        value: formatEntitlementValue(
          "can_auto_bump",
          plan.entitlements?.can_auto_bump,
        ),
      },
      {
        label: t("paymentsNew.companies", "Company creation"),
        value: formatEntitlementValue(
      ...
      {
        label: t("paymentsNew.tenders", "Tender access"),
        value: formatEntitlementValue(
          "can_view_tenders",
          plan?.entitlements?.can_view_tenders,
        ),
      },
      {
        label: t("paymentsNew.companies", "Company creation"),
      ...
      {
        label: t("paymentsNew.cargoLimit", "Cargo limit / month"),
        value: formatEntitlementValue(
          "cargo_limit",
      ...
      {
        label: t("paymentsNew.vehicleLimit", "Vehicle limit / month"),
        value: formatEntitlementValue(
          "vehicle_limit",
      ...
      {
        label: t("paymentsNew.activeTenders", "Active tenders"),
        value: formatEntitlementValue(
          "active_tenders",
          plan.entitlements?.active_tenders,
        ),
      },
    ],
    [plan, t],
    ...
    <Card
      ...
      sx={{
        ...
        "&:hover": {
          ...
          boxShadow: isCurrent
            ? "0 12px 28px rgba(46,125,50,0.12)"
            : "0 12px 24px rgba(15, 95, 194, 0.08)",
        ...
        <Box sx={{ mt: "auto" }}>
          <Tooltip
            title={
              isCurrent
                ? t(
                    "paymentsNew.tooltips.current",
                    "This is your current plan.",
                  )
                : t(
                    "paymentsNew.buttons.upgrade",
                    "Create an invoice to upgrade.",
                  )
            }
          >
