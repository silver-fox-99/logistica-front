...
} from "@/shared/config/entitlements";
import type {
  Entitlements,
  TariffPlan,
  TariffSubscription,
...
export default function TariffCard({ userId }: TariffCardProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const msg =
        e?.response?.data?.message ?? "Request failed. Please try again.";
      toast.error(msg);
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleEntitlements = async (payload: UpdateEntitlementsPayload) => {
    ...
    setEditLoading(true);
    try {
      await tariffsApi.adminUpdateSubscriptionEntitlements(
        targetSub.id,
        payload,
      );
        ...
        <Stack spacing={1.25}>
          ...
          >
            <Typography variant="body1" fontWeight={600}>
              Тариф:{" "}
              {active.plan?.name ?? planLookup.get(active.plan_id)?.name ?? "—"}{" "}
              (
              {active.plan?.code ??
                planLookup.get(active.plan_id)?.code ??
                active.plan_id}
              )
            </Typography>
