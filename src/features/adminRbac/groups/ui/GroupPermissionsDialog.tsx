...
import { MdExpandMore, MdOutlineSelectAll, MdOutlineRemoveDone, MdSearch } from "react-icons/md";
...
export const GroupPermissionsDialog = memo(function GroupPermissionsDialog({
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!alive) return;
                setError(e?.response?.data?.message ?? e?.message ?? "Не удалось загрузить права группы");
            } finally {
    ...
    const groups: PermissionGroup[] = useMemo(() => {
        ...
        const out: PermissionGroup[] = Array.from(map.entries()).map(([target, items]) => ({
            target,
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.message ?? e?.message ?? "Не удалось сохранить права");
        } finally {
            setSaving(false);
        }
    ...
    return (
        <Dialog open={open} onClose={saving ? undefined : onClose} fullWidth maxWidth="md">
            <DialogTitle sx={{ pb: 1 }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" gap={2}>
                    ...
                    <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap" justifyContent="flex-end">
                        ...
                        {group?.is_root ? <Chip size="small" color="warning" label="ROOT" /> : null}
                    </Stack>
                </Stack>
            </DialogTitle>
                        ...
                        {loading ? (
                            ...
                            <Stack gap={1}>
                                {groups.map((g) => {
                                    ...
                                    const selectedInGroup = g.items.reduce((acc, p) => acc + (selected.has(p.id) ? 1 : 0), 0);
                                    const allSelected = total > 0 && selectedInGroup === total;

                                    return (
                                        <Accordion key={g.target} defaultExpanded={defaultExpanded} disableGutters>
                                            <AccordionSummary expandIcon={<MdExpandMore />}>
                                                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ width: "100%" }}>
                                                    <Stack>
                                                        <Typography sx={{ fontWeight: 700 }}>{g.target}</Typography>
                                                        <Typography variant="caption" color="text.secondary">
                                                    ...
                                                    <Stack direction="row" gap={1} alignItems="center">
                                                        ...
                                                        <Button
                                                            ...
                                                            disabled={loading || saving || selectedInGroup === 0}
                                                        >
                                                            Снять
                                                        </Button>
                                                    </Stack>
            ...
            <DialogActions>
                ...
                <Button variant="contained" onClick={handleSave} disabled={loading || saving || !isDirty}>
                    Сохранить
                </Button>
            </DialogActions>
        </Dialog>
