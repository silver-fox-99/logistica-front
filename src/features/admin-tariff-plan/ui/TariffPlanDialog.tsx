...
export function TariffPlanDialog({
  ...
  const [form, setForm] = useState<TariffPlanFormValues>(
    createEmptyTariffPlanForm(),
  );
  ...
  return (
    ...
    >
      ...
      <DialogContent dividers sx={{ display: "grid", gap: 2, pt: 2 }}>
        <Alert severity="info">
          Заполните основные параметры тарифа. Все названия и ограничения должны
          быть понятны администратору, который будет работать с системой дальше.
        </Alert>
          ...
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
            ...
            <TextField
              ...
              onChange={(e) =>
                updateField(
                  "billing_period",
                  e.target.value as TariffPlanFormValues["billing_period"],
                )
              }
              sx={{ minWidth: 220 }}
        ...
        >
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
            Включите функции, которые будут доступны пользователям на этом
            тарифе.
          </Typography>
        ...
        >
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
            Укажите лимиты для этого тарифа. Если ограничение не нужно, оставьте
            поле пустым.
          </Typography>
          ...
          <Stack spacing={2}>
            {TARIFF_LIMIT_FIELDS.map((field) => (
              <TextField
                ...
                label={field.label}
                helperText={
                  field.hint || "Оставьте пустым, если лимит не нужен"
                }
                type="text"
                inputProps={{ inputMode: "numeric", pattern: "[0-9]*" }}
