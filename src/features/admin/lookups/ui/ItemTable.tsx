        ...
        <Card variant="outlined" sx={{ borderRadius: 2 }}>
            <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                    <Typography variant="h6" fontWeight={700}>{groupTitle ? `Items — ${groupTitle}` : "Items"}</Typography>
                    <Stack direction="row" spacing={1} alignItems="center">
                        <TextField size="small" placeholder="Quick filter…" onChange={(e) => onQuickFilter?.(e.target.value)} />
                        <Tooltip title="Reload"><IconButton onClick={onReload}><FiRefreshCw/></IconButton></Tooltip>
                        <Button startIcon={<FiPlus/>} variant="contained" onClick={onCreate}>New item</Button>
                    </Stack>
                </Stack>
                        ...
                        <Grid key={it.id} size={{ xs: 12 }}>
                            <Stack direction={{ xs: "column", sm: "row" }}
                                ...
                                <Stack spacing={0.25}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">slug: {it.slug} • order: {it.sort_order}</Typography>
                                </Stack>
                                <Stack direction="row" spacing={1} alignItems="center">
