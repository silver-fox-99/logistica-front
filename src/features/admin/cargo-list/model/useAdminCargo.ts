...
export function useAdminCargo(initial = { page: 1, limit: 20 }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const res = await adminCargoApi.list({ ...params, search: q || undefined });
                if (aborted) return;
                setItems(res.data);
                setTotal(res.total);
                setPages(res.pages);
            } catch (e: any) {
                if (!aborted) setError(e?.response?.data?.message ?? "Не удалось загрузить грузы");
            } finally {
    ...
    return {
        ...
        setSearch,
        refetch: () => setParams(p => ({ ...p })), // перезапрос
    };
}
