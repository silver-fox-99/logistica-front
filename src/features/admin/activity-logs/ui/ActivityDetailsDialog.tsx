        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
            ...
            <DialogContent dividers sx={{ display: "grid", gap: 2 }}>
                <Stack direction="row" spacing={2} flexWrap="wrap">
                    <Typography variant="body2"><b>Method:</b> {item?.method}</Typography>
                    <Typography variant="body2"><b>Endpoint:</b> {item?.endpoint}</Typography>
                    <Typography variant="body2"><b>Status:</b> {item?.statusCode}</Typography>
                    <Typography variant="body2"><b>Duration:</b> {item?.durationMs} ms</Typography>
                    <Typography variant="body2"><b>IP:</b> {item?.ip}</Typography>
                    <Typography variant="body2"><b>Time:</b> {item?.time ? new Date(item.time).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "medium", hour12: false }) : "—"}</Typography>
                    <Typography variant="body2"><b>User:</b> {item?.user ?? "—"}</Typography>
                </Stack>

                <Block title="Body" data={item?.body} />
                <Block title="Query" data={item?.query} />
                <Block title="Params" data={item?.params} />
