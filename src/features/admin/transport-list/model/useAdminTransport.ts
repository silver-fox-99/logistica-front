...
export function useAdminTransport(initial = { page: 1, limit: 20 }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const res = await adminTransportApi.list({ ...params, search: q || undefined });
                if (aborted) return;
                setItems(res.data);
                setTotal(res.total);
                setPages(res.pages);
    ...
    return {
        ...
        setSearch,
        refetch: () => setParams(p => ({ ...p })), // trigger reload
    };
}
