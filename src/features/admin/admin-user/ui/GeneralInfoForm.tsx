...
const buildDefaultValues = (u: AdminUser): FormValues => ({
  ...
  discount_percent:
    u.discount_percent !== null && u.discount_percent !== undefined
      ? String(u.discount_percent)
      : "",
  discount_expires_at: u.discount_expires_at
    ? u.discount_expires_at.slice(0, 10)
    : "",
...
export function GeneralInfoForm({
    ...
    try {
      const normalized: FormValues = {
        ...
        telegram_chat_id: values.telegram_chat_id?.trim()
          ? values.telegram_chat_id.trim()
          : null,
        discount_percent: values.discount_percent?.trim()
          ? String(Number(values.discount_percent))
          : null,
        discount_expires_at: values.discount_expires_at?.trim()
          ? new Date(values.discount_expires_at).toISOString()
          : null,
        discount_reason: values.discount_reason?.trim()
          ? values.discount_reason.trim()
          : null,
      };
      ...
      const payload = diffPayload<FormValues>(
        {
          ...
          discount_percent:
            user.discount_percent !== null &&
            user.discount_percent !== undefined
              ? String(user.discount_percent)
              : null,
      ...
      if (
        payload.phone &&
        (!phoneRegex.test(payload.phone) ||
          payload.phone.length < 10 ||
          payload.phone.length > 20)
      ) {
        toast.error(
          "Телефон должен быть в формате E.164 и содержать 10-20 символов",
        );
        setBusy(false);
        return;
      }
  ...
  const handleVerifyPhone = async () => {
    setBusy(true);
    try {
      await adminUserApi.patch(user.id, {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при верификации телефона";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const handleUnverifyPhone = async () => {
    ...
    try {
      ...
      const message =
        ...
        "Ошибка при сбросе верификации телефона";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при верификации email";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const handleUnverifyEmail = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при сбросе верификации email";
      toast.error(message);
    } finally {
      setBusy(false);
    }
            ...
            <Grid size={{ xs: 12 }}>
              <TextField
                ...
                {...register("avatar", {
                  setValueAs: (v) => (typeof v === "string" ? v.trim() : ""),
                  validate: (v) =>
                    !v || /^https?:\/\//i.test(v) || "Должен быть валидным URL",
                })}
                error={!!errors.avatar}
