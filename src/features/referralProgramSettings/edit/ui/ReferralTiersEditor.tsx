...
export function ReferralTiersEditor<TForm extends { tiers: TierFormValue[] }>(
  props: Props<TForm>,
) {
  ...
  const tiers = useWatch({ control, name: "tiers" as any }) as
    | TierFormValue[]
    | undefined;
  ...
  const issues = useMemo(() => {
    ...
    for (let i = 0; i < t.length; i++) {
      ...
      if (i === 0 && cur.from !== 0)
        rowErr.from = 'Первый тир должен начинаться с "0".';
      if (cur.from < 0 || !Number.isInteger(cur.from))
        rowErr.from = '"От" должно быть целым числом ≥ 0.';

      if (cur.to === null) {
        if (!isLast)
          rowErr.to = '"До" может быть пустым только у последнего тира.';
      } else {
        if (!Number.isInteger(cur.to) || cur.to < 0)
          rowErr.to = '"До" должно быть целым числом ≥ 0.';
        else if (cur.to < cur.from) rowErr.to = '"До" должно быть ≥ "От".';
      }

      if (!isNumericString(cur.reward_value ?? "")) {
        rowErr.reward =
          'Награда должна быть числом (например: "10" или "10.5").';
      }
  ...
  const addTier = () => {
    ...
    if (last.to === null) {
      setTopError(
        'Чтобы добавить новый тир, укажите конечное значение "До" для последнего тира.',
      );
      return;
    }
    ...
    <Stack spacing={1.5}>
      ...
      >
        ...
        <Button
          ...
          title={
            !canAdd
              ? 'Чтобы добавить новый тир, укажите "До" у последнего тира.'
              : undefined
          }
        >
          Добавить тир
      ...
      {fields.length === 0 && (
        <Box>
          <Typography variant="body2" color="text.secondary">
            Тиры не настроены (опционально). Если хотите использовать тиры —
            добавьте хотя бы один. Первый тир должен начинаться с 0.
          </Typography>
        </Box>
      )}
        ...
        {fields.map((f, idx) => {
          ...
          const rewardPreview =
            rewardType === "FIXED"
              ? formatIntWithDots(
                  String(
                    (tiers?.[idx]?.reward_value ?? "").replace(/[^\d]/g, "") ||
                      "0",
                  ),
                )
              : null;
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    onChange={(e) =>
                      field.onChange(parseOptionalInt(e.target.value))
                    }
                    error={!!rowIssues.to}
                    helperText={
                      rowIssues.to ??
                      (isLast
                        ? 'Оставьте пустым для "Без ограничений" (только в последнем тире).'
                        : "Обязательно.")
                    }
                    inputProps={{ min: 0, step: 1 }}
                  />
