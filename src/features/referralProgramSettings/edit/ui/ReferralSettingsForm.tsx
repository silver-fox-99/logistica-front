...
const ReferralSettingsForm = forwardRef<ReferralSettingsFormRef, Props>(
  function ReferralSettingsForm(props, ref) {
    const {
      settings,
      activeId,
      onChangeActiveId,
    ...
    function validateTiersClient(tiers: TierFormValue[]): string | null {
      ...
      for (let i = 0; i < tiers.length; i++) {
        ...
        if (!Number.isInteger(t.from) || t.from < 0)
          return `Tier #${i + 1}: "From" must be an integer ≥ 0.`;

        if (t.to === null) {
          if (!isLast)
            return `Tier #${i + 1}: "To" can be empty only for the last tier.`;
        } else {
          if (!Number.isInteger(t.to) || t.to < 0)
            return `Tier #${i + 1}: "To" must be an integer ≥ 0.`;
        ...
        if (i > 0) {
          ...
          if (prev.to === null)
            return `Tier #${i}: open-ended tier must be the last tier.`;
          const expectedFrom = prev.to + 1;
          if (t.from !== expectedFrom)
            return `Tier #${i + 1}: "From" must be ${expectedFrom}.`;
        }
      }
    ...
    const submit = handleSubmit(async (values) => {
      ...
      const dto: UpsertReferralSettingsDto = {
        ...
        reward_currency:
          values.reward_type === ReferralRewardType.FIXED
            ? values.reward_currency || "UZS"
            : null,
        meta: metaRes.data,
        document_id:
          values.linkMode === "DOCUMENT_ID" ? values.document_id : undefined,
        document_key:
          values.linkMode === "DOCUMENT_KEY" ? values.document_key : undefined,
        ...
        {settings.length > 1 && (
          <Stack direction="row" spacing={2}>
            <FormControl fullWidth>
              <InputLabel id="active-settings-label">
                Активные настройки
              </InputLabel>
              <Select
              ...
              >
                {settings.map((s) => (
                  <MenuItem key={s.id} value={s.id}>
                    {TRIGGER_RU[s.trigger] ?? s.trigger} •{" "}
                    {s.is_enabled ? "Включено" : "Отключено"}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                ...
                placeholder={
                  rewardType === ReferralRewardType.PERCENT ? "10.5" : "10000"
                }
                {...field}
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormControl fullWidth>
                ...
                <Select
                  labelId="link-mode-label"
                  label="Способ привязки"
                  {...field}
                >
                  <MenuItem value="NONE">Без документа</MenuItem>
                  <MenuItem value="DOCUMENT_ID">Привязать к документу</MenuItem>
                  <MenuItem value="DOCUMENT_KEY">
                    Привязать по ключу (последний опубликованный)
                  </MenuItem>
                </Select>
              </FormControl>
            )}
          ...
          <Controller
            ...
            render={({ field }) => {
              const valueObj = field.value
                ? (documents.find((d) => d.id === field.value) ?? null)
                : null;
