...
function extractCityAndCountry(item: PublicGeoLocationItem, lang: string) {
  ...
  return {
    city: city || "",
    country:
      country || (item.type === "COUNTRY" ? getLocalizedName(item, lang) : ""),
  };
}
...
export function RouteInterestSettings() {
  ...
  const [selectedOrigin, setSelectedOrigin] =
    useState<PublicGeoLocationItem | null>(null);
  ...
  const handleActivateMode = async (mode: "auto" | "manual") => {
    try {
      setSettingsLoading(true);
      const res = await profileApi.updateNotificationSettings({
        route_matching_mode: mode,
      });
      if (res?.status) {
        setMatchingMode(mode);
        toast.success(
          t(
            "notifications.routeInterests.modeActivateSuccess",
            "Режим отслеживания успешно изменен!",
          ),
        );
      }
    } catch (err) {
  ...
  const handleEnableAuto = async () => {
    ...
    if (ok) {
      ...
      toast.error(
        error || t("notifications.routeInterests.error", "Произошла ошибка"),
      );
    }
  };
  ...
  const handleAddManualRoute = async () => {
    ...
    const origin = selectedOrigin
      ? extractCityAndCountry(selectedOrigin, i18n.language)
      : { city: "", country: "" };
    const dest = selectedDest
      ? extractCityAndCountry(selectedDest, i18n.language)
      : { city: "", country: "" };
    ...
    if (ok) {
      toast.success(
        t(
          "notifications.routeInterests.manualAddedSuccess",
  ...
  const handleDeleteManualRoute = async (id: string) => {
    ...
    if (ok) {
      toast.success(
        t(
          "notifications.routeInterests.manualDeletedSuccess",
          "Маршрут удален из отслеживания",
        ),
      );
    } else {
      toast.error(
        ...
        <Stack direction="row" spacing={1.5} alignItems="center" mb={2}>
          ...
          <Stack spacing={0.5}>
            <Typography variant="h6" fontWeight={800} color="text.primary">
              {t(
                "notifications.routeInterests.cardTitle",
                "Умные уведомления по маршрутам",
              )}
            </Typography>
        ...
        >
          <Box
            ...
            sx={{
              ...
              userSelect: "none",
              bgcolor:
                activeTab === "auto" ? "background.paper" : "transparent",
              boxShadow:
                activeTab === "auto"
          ...
          >
            <FiLayers
              size={16}
              style={{
                transform: activeTab === "auto" ? "scale(1.1)" : "scale(1)",
            ...
            >
              ...
              >
                {t(
                  "notifications.routeInterests.tabAuto",
              ...
              {matchingMode === "auto" && (
                <Chip
                  label={t(
                    "notifications.routeInterests.modeActiveBadge",
                    "Активен",
                  )}
                  size="small"
                  color="success"
          ...
          <Box
            ...
            sx={{
              ...
              userSelect: "none",
              bgcolor:
                activeTab === "manual" ? "background.paper" : "transparent",
              boxShadow:
                activeTab === "manual"
                  ? theme.palette.mode === "dark"
                    ? "0 2px 8px rgba(0,0,0,0.4)"
          ...
          >
            <FiMapPin
              size={16}
              style={{
                transform: activeTab === "manual" ? "scale(1.1)" : "scale(1)",
                transition: "transform 0.2s",
              }}
            />
            <Typography
        ...
        {activeTab === "auto" ? (
          <Stack spacing={2.5}>
            {matchingMode !== "auto" && (
              <Alert
                ...
                action={
                  ...
                  >
                    {t(
                      "notifications.routeInterests.activateModeBtn",
              ...
              >
                {t(
                  "notifications.routeInterests.modeAlertManual",
                  "Внимание: сейчас у вас активен Ручной режим отслеживания. Автоматический подбор (ИИ) будет проигнорирован.",
                )}
              </Alert>
            )}
            ...
            {autoInterest ? (
              <Stack spacing={2.5}>
                {isSearching ? (
                  ...
                  >
                    ...
                    <Stack spacing={1} sx={{ maxWidth: 450 }}>
                      ...
                      >
                        {t(
                          "notifications.routeInterests.analyzingPreferences",
                    ...
                    >
                      {loading ? (
                        ...
                        t(
                          "notifications.routeInterests.disableAutoBtn",
                          "Отключить",
                        )
                      )}
                    </Button>
                  ...
                  >
                    ...
                    >
                      ...
                      <Stack>
                        ...
                        >
                          {t(
                            "notifications.routeInterests.autoActive",
                            "Автоматический подбор включен",
                          )}
                        </Typography>
                        <Typography
                          variant="body2"
                        ...
                        >
                          {t(
                            "notifications.routeInterests.detectedPreferences",
                            "Подобранное направление:",
                          )}{" "}
                          {[
                            autoInterest.origin_city,
                            autoInterest.origin_country,
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {t(
                            "notifications.routeInterests.autoActiveSub",
                            "Система слушает новые элементы",
                          )}{" "}
                          •{" "}
                          {t(
                            "notifications.routeInterests.threshold",
                            "Порог совпадения:",
                          )}{" "}
                          {autoInterest.match_percentage}%
                        </Typography>
                      </Stack>
                    </Stack>
                    <Button
                ...
                <Paper
                  variant="outlined"
                  sx={{ p: 2.5, borderRadius: "12px", borderColor: "divider" }}
                >
                  <Stack
                    direction={{ xs: "column", sm: "row" }}
                    spacing={3}
                  ...
                  >
                    <Box sx={{ flexGrow: 1, width: "100%", px: 1 }}>
                      ...
                      >
                        {t(
                          "notifications.routeInterests.matchPercentageThreshold",
                    ...
                    <Button
                      ...
                      disabled={
                        loading ||
                        autoMatchPct === autoInterest.match_percentage
                      }
                      sx={{
                        textTransform: "none",
                        borderRadius: "8px",
                    ...
                    >
                      {loading ? (
                        ...
                        t(
                          "notifications.routeInterests.saveThresholdBtn",
                          "Сохранить порог",
                        )
                      )}
                    </Button>
              ...
              >
                ...
                >
                  {t(
                    "notifications.routeInterests.autoInactive",
                    "Автоматический подбор не активен",
                  )}
                </Typography>
                <Typography
                  variant="body2"
                ...
                >
                  {loading ? (
                    ...
                    t(
                      "notifications.routeInterests.enableAutoBtn",
                      "Включить автоподбор",
                    )
                  )}
                </Button>
          ...
          <Stack spacing={3}>
            {matchingMode !== "manual" && (
              ...
              >
                {t(
                  "notifications.routeInterests.modeAlertAuto",
                  "Внимание: сейчас у вас активен Автоматический режим подбора (ИИ). Ручные настройки маршрутов будут проигнорированы.",
                )}
              </Alert>
            )}
            <Typography variant="body2" color="text.secondary" lineHeight={1.6}>
            ...
            <Stack spacing={3}>
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t(
                        "notifications.routeInterests.originLabel",
                        "Откуда (Город/Страна)",
                      )}
                      placeholder={t(
                        "notifications.routeInterests.originPlaceholder",
                        "Начните вводить...",
                      )}
                      InputProps={{
                        ...params.InputProps,
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t(
                        "notifications.routeInterests.destLabel",
                        "Куда (Город/Страна)",
                      )}
                      placeholder={t(
                        "notifications.routeInterests.destPlaceholder",
                        "Начните вводить...",
                      )}
                      InputProps={{
                        ...params.InputProps,
              ...
              >
                {loading ? (
                  ...
                  t(
                    "notifications.routeInterests.addRouteBtn",
                    "Добавить маршрут",
                  )
                )}
              </Button>
            ...
            <Box>
              ...
              >
                {t(
                  "notifications.routeInterests.trackedRoutesTitle",
                  "Отслеживаемые маршруты",
                )}{" "}
                ({manualInterests.length})
              </Typography>
                ...
                >
                  <List disablePadding>
                    {manualInterests.map((interest, index) => {
                      const originText = [
                        interest.origin_city,
                        interest.origin_country,
                      ]
                        .filter(Boolean)
                        .join(", ");
                      const destText = [
                        interest.destination_city,
                        interest.destination_country,
                      ]
                        .filter(Boolean)
                        .join(", ");
                      return (
                        <React.Fragment key={interest.id}>
                          ...
                          >
                            ...
                            <IconButton
                              color="error"
                              onClick={() =>
                                handleDeleteManualRoute(interest.id)
                              }
                              disabled={loading}
                              sx={{
