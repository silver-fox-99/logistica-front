...
export function ChangePasswordCard() {
  ...
  const onSubmit = async ({ oldPassword, newPassword }: FormValues) => {
    ...
    try {
      ...
      const message =
        e?.response?.data?.message ?? t("security.changePassword.errorMessage");
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      <Box sx={{ p: { xs: 2, sm: 2.5 } }}>
        ...
        <Stack spacing={2} component="form" onSubmit={handleSubmit(onSubmit)}>
              ...
              endAdornment: (
                <InputAdornment position="end">
                  <IconButton
                    ...
                    edge="end"
                    aria-label={t("security.changePassword.togglePassword")}
                  >
          ...
          <Box sx={{ pt: 0.5 }}>
            ...
            >
              {loading
                ? t("security.changePassword.pleaseWait")
                : t("security.changePassword.changeButton")}
            </Button>
          </Box>
        </Stack>
      </Box>
