        ...
        <Stack direction="row" flexWrap="wrap" gap={1}>
          {[
            ...
            <Chip
              ...
              sx={{
                borderColor:
                  ratingFilter === item.value ? "primary.main" : "divider",
                borderRadius: 1,
              }}
            />
          ))}
...
function ReviewCard({
  ...
  const routeFrom =
    [data.pickup_country, data.pickup_region, data.pickup_city]
      ...
      .join(", ") ||
    [
      geoName(data.pickup_country_id),
      geoName(data.pickup_region_id),
      geoName(data.pickup_city_id),
    ]
      .filter(Boolean)
      .join(", ") ||
    data.pickup_city_id ||
  ...
  const routeTo =
    [data.dropoff_country, data.dropoff_region, data.dropoff_city]
      ...
      .join(", ") ||
    [
      geoName(data.dropoff_country_id),
      geoName(data.dropoff_region_id),
      geoName(data.dropoff_city_id),
    ]
      .filter(Boolean)
      .join(", ") ||
  ...
  const price =
    data.price_amount != null && data.price_amount !== 0 && data.price_currency
        ...
        <Stack direction="row" spacing={1.5} alignItems="center">
          <Avatar
            sx={{
              width: 40,
              height: 40,
              cursor: data.from_user_id ? "pointer" : "default",
            }}
            onClick={() => {
