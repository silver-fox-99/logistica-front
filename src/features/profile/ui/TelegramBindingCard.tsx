...
export default function TelegramBindingCard() {
  ...
  const checkBindingStatus = async (showFeedback = true) => {
    ...
    try {
      ...
      if (showFeedback) {
        toast.error(
          "Не удалось проверить статус привязки. Попробуйте еще раз.",
        );
      }
    } finally {
      setChecking(false);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка подключения Telegram";
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      <Stack spacing={2}>
        ...
        >
          ...
          <Chip
            icon={
              isConnected ? (
                <FiCheckCircle style={{ color: "inherit" }} />
              ) : (
                <FiInfo style={{ color: "inherit" }} />
              )
            }
            label={
              isConnected
                ? t("profile.telegramBinding.connected")
                : t("profile.telegramBinding.notConnected")
            }
        ...
        >
          ...
          >
            {isWaiting && !isConnected && (
              ...
              >
                {checking
                  ? t("profile.telegramBinding.checkingStatus")
                  : t("profile.telegramBinding.checkStatus")}
              </Button>
            )}
            ...
            {!isConnected && (
              ...
              >
                {loading
                  ? t("profile.telegramBinding.connecting")
                  : t("profile.telegramBinding.connect")}
              </Button>
            )}
          </Stack>
        </Box>
