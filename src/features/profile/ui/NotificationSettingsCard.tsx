...
import {
    Paper, Stack, Typography, Switch, Box, CircularProgress, Divider
} from "@mui/material";
import { useTranslation } from "react-i18next";
import {
    FiBell, FiPlusSquare, FiTrendingUp, FiAward, FiCheckCircle,
    FiMapPin, FiGift, FiSmartphone, FiMail, FiMonitor
} from "react-icons/fi";
            ...
            <Stack spacing={3}>
                ...
                <Stack spacing={2}>
                    <Typography variant="subtitle1" fontWeight={700} color="text.primary">
                        {t("profile.notifications.channelsTitle", "Каналы получения уведомлений")}
                    </Typography>
                    ...
                    >
                        <Stack direction="row" spacing={2} alignItems="center" sx={{ pr: 2 }}>
                            ...
                            <Stack spacing={0.2}>
                                ...
                                <Typography variant="body2" color="text.secondary">
                                    {t("profile.notifications.channelInAppDesc", "Мгновенные уведомления (колокольчик) в шапке сайта")}
                                </Typography>
                            </Stack>
                        </Stack>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            {savingField === "channel_in_app" && <CircularProgress size={16} />}
                            <Switch
                    ...
                    >
                        <Stack direction="row" spacing={2} alignItems="center" sx={{ pr: 2 }}>
                            ...
                            <Stack spacing={0.2}>
                                ...
                                <Typography variant="body2" color="text.secondary">
                                    {isTelegramConnected 
                                        ? t("profile.notifications.channelTelegramDesc", "Личные уведомления в привязанный аккаунт Telegram")
                                        : t("profile.notifications.channelTelegramRequired", "Необходимо привязать Telegram-аккаунт в блоке выше")
                                    }
                                </Typography>
                            </Stack>
                        </Stack>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            {savingField === "channel_telegram" && <CircularProgress size={16} />}
                            <Switch
                                checked={isTelegramConnected && (settings?.channel_telegram ?? true)}
                                onChange={() => void handleToggle("channel_telegram")}
                                disabled={!isTelegramConnected || savingField === "channel_telegram"}
                                color="primary"
                            />
                        </Box>
                    </Box>
                    ...
                    >
                        <Stack direction="row" spacing={2} alignItems="center" sx={{ pr: 2 }}>
                            ...
                            <Stack spacing={0.2}>
                                ...
                                <Typography variant="body2" color="text.secondary">
                                    {isEmailVerified
                                        ? t("profile.notifications.channelEmailDesc", "Важные сводки и обновления на ваш подтвержденный email")
                                        : t("profile.notifications.channelEmailRequired", "Необходимо подтвердить E-mail в блоке выше")
                                    }
                                </Typography>
                            </Stack>
                        </Stack>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            {savingField === "channel_email" && <CircularProgress size={16} />}
                            <Switch
                ...
                <Stack spacing={2}>
                    <Typography variant="subtitle1" fontWeight={700} color="text.primary">
                        {t("profile.notifications.eventsTitle", "О каких событиях уведомлять")}
                    </Typography>
                        ...
                        <Stack spacing={2}>
                            ...
                            >
                                <Stack direction="row" spacing={2} alignItems="center" sx={{ pr: 2 }}>
                                    ...
                                    <Stack spacing={0.2}>
                                        <Typography variant="subtitle2" fontWeight={700}>
                                            {t("profile.notifications.routeMatches", "Интересные маршруты")}
                                        </Typography>
                                        <Typography variant="body2" color="text.secondary">
                                            {t("profile.notifications.routeMatchesDesc", "Появление нового транспорта/грузов по маршрутам, которые вы часто ищете")}
                                        </Typography>
                                    </Stack>
                                </Stack>
                                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                    {savingField === "notify_route_matches" && <CircularProgress size={16} />}
                                    <Switch
                            ...
                            >
                                <Stack direction="row" spacing={2} alignItems="center" sx={{ pr: 2 }}>
                                    ...
                                    <Stack spacing={0.2}>
                                        <Typography variant="subtitle2" fontWeight={700}>
                                            {t("profile.notifications.systemUpdates", "Новости и скидки")}
                                        </Typography>
                                        <Typography variant="body2" color="text.secondary">
                                            {t("profile.notifications.systemUpdatesDesc", "Скидки на тарифы, обновления сайта, спецпредложения и новости платформы")}
                                        </Typography>
                                    </Stack>
                                </Stack>
                                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                    {savingField === "notify_system_updates" && <CircularProgress size={16} />}
                                    <Switch
