...
export function ShipmentDetailsContent({
  ...
  const loadTypeArray = Array.isArray(data.loadType)
    ? data.loadType
    : data.loadType
      ? [data.loadType]
      : [];
  ...
  return (
    <Grid container spacing={2}>
      <Grid size={{ xs: 12, md: 6 }}>
        <DetailSection
          title={t("shipments.shipmentCard.contactInfo", "Contact information")}
        >
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <DetailSection
          title={t("shipments.shipmentCard.orderDetails", "Order details")}
        >
          <Stack spacing={0.75} color="text.secondary">
            ...
            {data.carsCount != null && data.carsCount > 0 && (
              <Typography variant="body2">
                <strong>
                  {t("shipments.shipmentCard.carsCount", "Cars count")}
                </strong>{" "}
                {data.carsCount}
              </Typography>
            )}
            ...
            {data.palletsCount != null && data.palletsCount > 0 && (
              <Typography variant="body2">
                <strong>
                  {t("shipments.shipmentCard.palletsCount", "Pallets count")}
                </strong>{" "}
                {data.palletsCount}
              </Typography>
            )}
            ...
            {data.bargain && (
              <Typography variant="body2">
                <strong>
                  {t("shipments.shipmentCard.bargain", "Bargain")}
                </strong>{" "}
                {data.bargain === "ALLOWED"
                  ? t("shipments.shipmentCard.bargainAllowed", "Allowed")
                  : t(
                      "shipments.shipmentCard.bargainNotAllowed",
                      "Not allowed",
                    )}
              </Typography>
            )}
