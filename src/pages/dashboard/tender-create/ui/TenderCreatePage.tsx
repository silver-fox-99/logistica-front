...
import { Controller, useFieldArray, useForm, type Control, type UseFormReturn, type UseFormSetValue } from "react-hook-form";
import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import { FiArrowDown, FiArrowUp, FiPlus, FiTrash2 } from "react-icons/fi";

import { TenderAuctionType, TenderPointType, type CreateTenderPayload, type TenderPoint } from "@/entities/tender/model/types";
...
import type { Place, PlaceLocation } from "@/features/add-cargo-form/model/types";
...
function toTenderPoint(point: Place, type: TenderPointType, index: number): TenderPoint {
...
function TenderPlaceRowField({ kind, index, control, setValue, errorText }: TenderPlaceRowFieldProps) {
        ...
        <Stack spacing={1.25}>
                ...
                render={({ field }) => (
                    <Autocomplete
                        ...
                        onChange={(_, value: any) => {
                            ...
                            setValue(`${fieldName}.address`, "", { shouldDirty: true, shouldValidate: false });
                        }}
                        renderInput={(params) => (
                            <TextField
                                {...params}
                                label={kind === "pickup" ? t("tenders.create.pickupPoint") : t("tenders.create.dropoffPoint")}
            ...
            <Controller
                ...
                render={({ field }) => (
                    <TextField
                        {...field}
                        label={kind === "pickup" ? t("tenders.create.pickupAddress") : t("tenders.create.dropoffAddress")}
...
function TenderPointsFieldArray({ kind, name, form, errorMessages = [] }: TenderPointsFieldArrayProps) {
    const { t } = useTranslation();
    const { control, setValue } = form;
    const { fields, append, remove, move } = useFieldArray({ control, name });
    const title = kind === "pickup" ? t("tenders.create.pickupPoints") : t("tenders.create.dropoffPoints");
    const itemTitle = kind === "pickup" ? t("tenders.create.pickupPoint") : t("tenders.create.dropoffPoint");
    ...
    return (
        <Stack spacing={1.5}>
                    ...
                    <Stack spacing={1.25}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
                            ...
                            <Stack direction="row" flexWrap="wrap" gap={1}>
                                <Button size="small" variant="text" startIcon={<FiArrowUp />} onClick={() => move(index, index - 1)} disabled={index === 0}>
                                    {t("tenders.create.moveUp")}
                                </Button>
                                <Button size="small" variant="text" startIcon={<FiArrowDown />} onClick={() => move(index, index + 1)} disabled={index === fields.length - 1}>
                                    {t("tenders.create.moveDown")}
                                </Button>
                                <Button size="small" color="error" variant="text" startIcon={<FiTrash2 />} onClick={() => remove(index)} disabled={fields.length === 1}>
            ...
            <Box>
                <Button variant="outlined" startIcon={<FiPlus />} onClick={() => append(EMPTY_PLACE)}>
                    {kind === "pickup" ? t("tenders.create.addPickupPoint") : t("tenders.create.addDropoffPoint")}
                </Button>
            </Box>
        </Stack>
    );
...
export default function TenderCreatePage() {
    ...
    const { register, control, setValue, getValues, setError, clearErrors, formState, handleSubmit } = form;

    useEffect(() => {
        if (!lookups || loadingInit) return;

        if (!getValues("currency")) setValue("currency", lookups.currency?.[0]?.slug ?? "USD", { shouldDirty: false });
        if (!getValues("vehicleType")) setValue("vehicleType", lookups.vehicleType?.[0]?.slug ?? "ANY", { shouldDirty: false });
        if (!getValues("cargoType")) setValue("cargoType", lookups.cargoTypes?.[0]?.slug ?? "GENERAL", { shouldDirty: false });
    ...
    const validatePlaces = useCallback(
        (places: Place[], fieldName: "pickups" | "dropoffs", message: string) => {
            let ok = true;
            if (!places?.length) {
                setError(`${fieldName}.0.location` as any, { type: "required", message });
                return false;
            }

            places.forEach((place, index) => {
                if (!place.location) {
                    setError(`${fieldName}.${index}.location` as any, { type: "required", message });
                    ok = false;
                }
            });
    ...
    const buildPayload = useCallback((values: TenderCreateFormValues): CreateTenderPayload => {
        const pickups = (values.pickups ?? []).filter((item) => !!item.location);
        const dropoffs = (values.dropoffs ?? []).filter((item) => !!item.location);
        ...
        return {
            ...
            phone,
            points: [
                ...pickups.map((point, index) => toTenderPoint(point, TenderPointType.PICKUP, index)),
                ...dropoffs.map((point, index) => toTenderPoint(point, TenderPointType.DROPOFF, index)),
            ],
        };
    }, []);
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        if (!values.title.trim()) {
            setError("title", { type: "required", message: t("tenders.settings.validation.title") });
            ok = false;
        }
        if (!values.startPrice.trim()) {
            setError("startPrice", { type: "required", message: t("tenders.settings.validation.startPrice") });
            ok = false;
        }
        if (!values.startsAt) {
            setError("startsAt", { type: "required", message: t("tenders.create.startRequired") });
            ok = false;
        }
        if (!values.endsAt) {
            setError("endsAt", { type: "required", message: t("tenders.create.endRequired") });
            ok = false;
        }
        if (values.startsAt && values.endsAt && values.endsAt <= values.startsAt) {
            setError("endsAt", { type: "validate", message: t("tenders.settings.validation.dateOrder") });
            ok = false;
        }
        ...
        if (!validatePlaces(values.pickups, "pickups", t("tenders.create.selectPickup"))) ok = false;
        if (!validatePlaces(values.dropoffs, "dropoffs", t("tenders.create.selectDropoff"))) ok = false;
        ...
        try {
            ...
            toast.error(error?.response?.data?.message || t("tenders.create.createError"));
        } finally {
            setLoading(false);
        }
    });

    const pickupErrors = ((formState.errors.pickups ?? []) as any[]).map((item) => item?.location?.message as string | undefined) ?? [];
    const dropoffErrors = ((formState.errors.dropoffs ?? []) as any[]).map((item) => item?.location?.message as string | undefined) ?? [];
                ...
                <Stack spacing={3} component="form" noValidate onSubmit={onSubmit}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.pickupDate")} type="date" InputLabelProps={{ shrink: true }} fullWidth {...register("pickupDate")} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.dropoffDate")} type="date" InputLabelProps={{ shrink: true }} fullWidth {...register("dropoffDate")} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.pickupTime")} type="time" InputLabelProps={{ shrink: true }} fullWidth {...register("pickupTime")} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.dropoffTime")} type="time" InputLabelProps={{ shrink: true }} fullWidth {...register("dropoffTime")} />
                        </Grid>

                        <Grid size={{ xs: 12, md: 6 }}>
                            <TenderPointsFieldArray kind="pickup" name="pickups" form={form} errorMessages={pickupErrors} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TenderPointsFieldArray kind="dropoff" name="dropoffs" form={form} errorMessages={dropoffErrors} />
                        </Grid>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.placesCount")} fullWidth {...register("placesCount")} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField label={t("tenders.fields.weightTons")} fullWidth {...register("weightTons")} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                            <FormControlLabel
                                control={<Controller control={control} name="adrRequired" render={({ field }) => <Checkbox checked={field.value} onChange={(e) => field.onChange(e.target.checked)} />} />}
                                label={t("tenders.create.adr")}
                            />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
                            <FormControlLabel
                                control={<Controller control={control} name="hydraulicTailLiftRequired" render={({ field }) => <Checkbox checked={field.value} onChange={(e) => field.onChange(e.target.checked)} />} />}
                                label={t("tenders.create.hydraulicTailLiftRequired")}
                            />
                        </Grid>
                        <Grid size={{ xs: 12 }}>
                            <TextField label={t("tenders.fields.cargoDescription")} multiline minRows={3} fullWidth {...register("cargoDescription")} />
                        </Grid>
