...
import ContactInfoCard, {
  type ContactInfo,
} from "@/features/profile/ui/ContactInfoCard.tsx";
...
export default function ProfilePage() {
  ...
  const [membershipHistory, setMembershipHistory] = useState<
    CompanyMembershipHistoryItem[]
  >([]);
  const [membershipHistoryLoading, setMembershipHistoryLoading] =
    useState(false);
  ...
  const loadSelfReviews = useCallback(async () => {
    ...
    try {
      const data = await userReviewsApi.list(user.id, {
        page: 1,
        limit: 20,
        sort: "new",
      });
  ...
  const loadMembershipHistory = useCallback(async () => {
    ...
    try {
      ...
      const message =
        ...
        t(
          "profile.membershipHistory.loadError",
...
function ProfileReviewRow({ review }: { review: UserReview }) {
  ...
  const routeFrom =
    [review.pickup_country, review.pickup_region, review.pickup_city]
      .filter(Boolean)
      .join(", ") ||
    review.pickup_city ||
  ...
  const routeTo =
    [review.dropoff_country, review.dropoff_region, review.dropoff_city]
      .filter(Boolean)
      .join(", ") ||
    review.dropoff_city ||
