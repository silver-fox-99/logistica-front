...
function routeLabel(tender: Tender) {
    ...
    const to = dropoffs[dropoffs.length - 1] ?? tender.points?.[tender.points.length - 1];
        ...
        >
            <Stack spacing={1.25}>
                <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1}>
                    <Stack spacing={0.5}>
                        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                            ...
                            />
                            {tender.buyout_price && (
                                <Chip size="small" color="success" variant="outlined" label={t("tenders.list.card.buyout")} />
                            )}
                        </Stack>
                ...
                <Grid container spacing={1.5}>
                    <Grid size={{ xs: 6, md: 3 }}>
                        <Typography variant="caption" color="text.secondary">{t("tenders.list.card.endsAt")}</Typography>
                    ...
                    <Grid size={{ xs: 6, md: 3 }}>
                        ...
                        <Typography variant="body2">
                            {tender.weight_t || t("tenders.common.empty")} t / {tender.volume_m3 || t("tenders.common.empty")} m3
                        </Typography>
                    </Grid>
                    <Grid size={{ xs: 6, md: 3 }}>
                        <Typography variant="caption" color="text.secondary">{t("tenders.list.card.bids")}</Typography>
                        <Typography variant="body2">
                            {bidsCount || (tender.has_bids ? t("tenders.list.card.hasBids") : t("tenders.list.card.noBids"))}
                        </Typography>
                    </Grid>
                </Grid>
            </Stack>
        ...
        try {
            ...
            if (scope === "my" && myTab === "active") {
                const [wins, incomplete, cachedOwnerTenders] = await Promise.all([
                    tendersApi.myWins(params).catch(() => ({ data: [], total: 0, page, limit })),
                    tendersApi.myIncomplete(params).catch(() => ({ data: [], total: 0, page, limit })),
                    Promise.all(
                ...
                const waitingConfirmation = [
                    ...wins.data,
                    ...incomplete.data,
                    ...cachedOwnerTenders.filter((tender): tender is Tender => Boolean(tender)),
                ].filter((tender) => {
                    const belongsToCurrentOwner = currentUserId && tender.owner_id === currentUserId;
                    const belongsToCurrentWinner = currentUserId && tender.current_winner_id === currentUserId;
                ...
                for (const tender of waitingConfirmation) {
                    if (!merged.some((item) => item.id === tender.id)) merged.push(tender);
                }
                setItems(merged);
            ...
            if (scope === "my" && myTab === "archive" && e?.response?.status === 404) {
                setItems([]);
                setTotal(0);
