...
import {
  ...
  type FilterSettings,
  type FilterConfig,
} from "@/shared/store/filterSettingsStore";
...
function configFromForm(form: FilterConfigForm): FilterConfig {
  ...
  if (form.weight_min !== undefined && form.weight_min !== null)
    out.weight_min = form.weight_min;
  if (form.weight_max !== undefined && form.weight_max !== null)
    out.weight_max = form.weight_max;
  if (form.volume_min !== undefined && form.volume_min !== null)
    out.volume_min = form.volume_min;
  if (form.volume_max !== undefined && form.volume_max !== null)
    out.volume_max = form.volume_max;

  if (form.vehicle_type && form.vehicle_type.length > 0)
    out.vehicle_type = form.vehicle_type;
      ...
      <Stack direction="row" spacing={1} alignItems="center">
        ...
        <Controller
          ...
          render={({ field: modeField }) =>
            modeField.value === "today_plus" ||
            modeField.value === "today_minus" ? (
              <Controller
                control={control}
                name={`${name}.offset`}
                render={({ field }) => (
...
export default function AdminSystemSettingsPage() {
  ...
  const { settings, loading, error, loadSettings, saveSettings } =
    useFilterSettingsStore();

  const [activeTab, setActiveTab] = useState<"search" | "my" | "home">(
    "search",
  );
  const canViewSettings = useAdminAccessStore((s) =>
    s.hasPermission(viewCode("SYSTEM_SETTINGS" as any)),
  );
  ...
  const handleSave = handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || "Ошибка при сохранении настроек",
      );
    }
  });
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{ mb: 3 }}
                  >
                    Применяются автоматически при первом открытии страницы
                    пользователем.
                  </Typography>
                  <FilterConfigPanel
                    control={control}
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{ mb: 3 }}
                  >
                    Применяются, когда пользователь нажимает кнопку «Сбросить» в
                    панели фильтров.
                  </Typography>
                  <FilterConfigPanel
                    control={control}
