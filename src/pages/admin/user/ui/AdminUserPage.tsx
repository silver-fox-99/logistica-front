...
export default function AdminUserPage() {
  ...
  const { user, sessions, groups, loading, err, setUser, setGroups } =
    useAdminUser(id);
  const setCurrentUser = useUserStore((s) => s.setUser);
  const [impersonating, setImpersonating] = useState(false);
  const [tabValue, setTabValue] = useState(0);
  const canViewUserDetails = useAdminAccessStore((s) =>
    s.hasPermission(viewCode("USER_DETAILS" as any)),
  );
  ...
  const handleImpersonate = async () => {
    ...
    try {
      ...
      const message =
        ...
        "Не удалось выполнить вход под пользователем";
      toast.error(message);
    } finally {
  ...
  if (!user)
    return (
      <Container sx={{ py: 4 }}>
        <Alert severity="error">{err ?? " Пользователь не найден"}</Alert>
      </Container>
    );
