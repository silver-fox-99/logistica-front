...
import {
  adminIpBlacklistApi,
  type IpBan,
} from "@/shared/api/adminIpBlackListApi.ts";
import IpBanDialog, {
  type IpBanFormValues,
} from "@/features/admin/ip-blacklist/ui/IpBanDialog";
...
export default function AdminBlacklistPage() {
  ...
  const canViewBalckList = useAdminAccessStore((s) =>
    s.hasPermission(viewCode("BLACKLIST" as any)),
  );
  ...
  const createSubmit = async (v: IpBanFormValues) => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при добавлении IP";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const remove = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при удалении IP";
      toast.error(message);
    } finally {
      setBusy(false);
    }
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell>
                    <Chip size="small" label={row.network} />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {row.created_by ? (
                      <Stack
                        ...
                        sx={{ cursor: "pointer" }}
                        onClick={() =>
                          navigate(`/admin/user/${row.created_by!.id}`)
                        }
                      >
                        <Avatar
                          src={row.created_by.avatar ?? undefined}
