...
function ActivityFiltersBar({
  ...
  const canViewActivityLogs = useAdminAccessStore((s) =>
    s.hasPermission(viewCode("ACTIVITY_LOGS" as any)),
  );
    ...
    <Paper variant="outlined" sx={{ p: 1.25, borderRadius: 2 }}>
      ...
      >
        ...
        <Select
          ...
          displayEmpty
          onChange={(e) =>
            setFilters((f) => ({ ...f, method: String(e.target.value) }))
          }
          sx={{ minWidth: 150 }}
        >
          <MenuItem value="">
        ...
        <Stack direction="row" alignItems="center" spacing={1} sx={{ px: 0.5 }}>
          <Switch
            ...
            onChange={(_, v) =>
              setFilters((f) => ({ ...f, includeAnonymous: v }))
            }
            size="small"
          />
...
export default function AdminActivityLogsPage() {
  ...
  const openDetails = (row: (typeof items)[number]) => {
    setSelected(row);
    setOpen(true);
  };
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell>
                    <Chip size="small" label={row.method} />
                  </TableCell>
                  <TableCell>
