...
export default function AdminCargoPage() {
  ...
  const canViewCargo = useAdminAccessStore((s) =>
    s.hasPermission(viewCode("CARGO" as any)),
  );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при удалении груза";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const confirmRestore = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при восстановлении груза";
      toast.error(message);
    } finally {
      setBusy(false);
    }
  ...
  const handleReactivate = async (id: string) => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || "Ошибка при активации груза";
      toast.error(message);
    } finally {
      setBusy(false);
    }
                  ...
                  >
                    <TableCell>
                      <Stack spacing={0.5}>
                        {isDeleted ? (
                          <>
                            <Chip size="small" color="error" label="Удалён" />
                            <Typography
                              variant="caption"
                    ...
                    <TableCell>
                      ...
                      >
                        ...
                        />
                        {c.has_dimensions && (
                          <Chip size="small" color="primary" label="Размеры" />
                        )}
