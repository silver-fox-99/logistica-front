        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth scroll="paper" PaperProps={{ sx: { borderRadius: 3 } }}>
            <form onSubmit={(e) => void handleFormSubmit(e)}>
                ...
                <DialogContent dividers>
                    <Stack spacing={2.5} sx={{ py: 1 }}>
                        ...
                        <FormControl fullWidth size="small">
                            ...
                            >
                                <MenuItem value="SYSTEM_UPDATE">Обновление системы / Объявление</MenuItem>
                        ...
                        <Alert severity="info" sx={{ borderRadius: 2, "& .MuiAlert-message": { width: "100%" } }}>
                            ...
                            <Typography variant="caption" color="text.secondary" display="block" sx={{ lineHeight: 1.4 }}>
                                Они позволяют добавлять интерактивные кнопки или яркие плашки к уведомлению:
                                <br />
                                • <strong>"link"</strong> — создаст кнопку перехода в нужный раздел (например, в оплату или тендеры).
                                <br />
                                • <strong>"badge"</strong> — отобразит цветной текстовый ярлык (например, "АКЦИЯ" или "ВАЖНО") рядом с заголовком.
                            </Typography>
                        </Alert>
                        ...
                        <Box>
                            ...
                            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ gap: 1 }}>
                                <Chip
                                    ...
                                    clickable
                                    variant="outlined"
                                    onClick={() => setRawMetadata(JSON.stringify({ link: "/dashboard/payments" }, null, 4))}
                                    sx={{ borderRadius: 1.5 }}
                                />
                                <Chip
                                    icon={<FiLink size={12} />}
                                    ...
                                    clickable
                                    variant="outlined"
                                    onClick={() => setRawMetadata(JSON.stringify({ link: "/dashboard/tenders" }, null, 4))}
                                    sx={{ borderRadius: 1.5 }}
                                />
                                <Chip
                                    icon={<FiTag size={12} />}
                                    ...
                                    clickable
                                    variant="outlined"
                                    onClick={() => setRawMetadata(JSON.stringify({ badge: "АКЦИЯ" }, null, 4))}
                                    sx={{ borderRadius: 1.5 }}
                                />
                                <Chip
                                    icon={<FiTag size={12} />}
                                    ...
                                    clickable
                                    variant="outlined"
                                    onClick={() => setRawMetadata(JSON.stringify({ badge: "ВАЖНО" }, null, 4))}
                                    sx={{ borderRadius: 1.5 }}
                                />
                                <Chip
                ...
                <DialogActions sx={{ px: 3, py: 2 }}>
                    <Button onClick={onClose} color="inherit" disabled={loading} sx={{ textTransform: "none", borderRadius: 2 }}>
                        Отмена
                    </Button>
                    <Button
                        type="submit"
