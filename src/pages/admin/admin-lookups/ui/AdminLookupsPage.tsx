...
export default function AdminLookupsPage() {
    ...
    const moveItem = async (id: string, dir: "up"|"down") => {
        ...
        const copy = [...items];
        const a = copy[idx], b = copy[otherIdx];
        ...
        setItems(copy);
        if (current) await lookupsApi.reorderItems(current.code, copy.map(it => ({ id: it.id, sort_order: it.sort_order })));
    };

    const onQuickFilter = (text: string) => {
        const t = text.trim().toLowerCase();
        if (!t) { void reloadItems(); return; }
        const filtered = items.filter(i => i.label.toLowerCase().includes(t) || i.slug.toLowerCase().includes(t));
        setItems(filtered);
    };
