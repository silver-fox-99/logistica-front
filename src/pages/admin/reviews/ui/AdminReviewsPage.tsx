...
export default function AdminReviewsPage() {
    ...
    const fetchUserReviews = useCallback(
        async (nextPage = 1) => {
            ...
            try {
                ...
                toast.error(e?.message || "Не удалось загрузить отзывы о пользователях");
                setUserReviews([]);
            } finally {
                setLoading(false);
            }
    ...
    const refreshData = useCallback((targetPage?: number) => {
        if (tab === "users") {
    ...
    const updateUserReviewStatus = async (review: UserReview, nextStatus: "PUBLISHED" | "REJECTED") => {
        const targetId =
            review.to_user_id || (review as any).user_id || (review as any).toUserId || review.from_user_id;
        if (!targetId) {
        ...
        try {
            ...
            setUserReviews((prev) => prev.map((r) => (r.id === review.id ? { ...r, status: nextStatus } : r)));
    ...
    const updateSiteReviewStatus = async (review: SiteReview, nextStatus: "APPROVED" | "REJECTED") => {
        try {
            await siteReviewsApi.adminModerate(review.id, { status: nextStatus });
            setSiteReviews((prev) => prev.map((r) => (r.id === review.id ? { ...r, status: nextStatus } : r)));
            toast.success(nextStatus === "APPROVED" ? "Отзыв опубликован" : "Отзыв отклонен");
                                                ...
                                                <TableCell align="right">
                                                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                                                        <IconButton
                                                            size="small"
                                                            title="Опубликовать"
                                                            onClick={() => updateUserReviewStatus(r, "PUBLISHED")}
                                                        >
                                                            <FiCheck />
                                                        </IconButton>
                                                        <IconButton
                                                            size="small"
                                                            title="Отклонить"
                                                            onClick={() => updateUserReviewStatus(r, "REJECTED")}
                                                        >
                                                            <FiX />
                                                        </IconButton>
                                                        <IconButton
                                ...
                                {loading ? (
                                    ...
                                    siteReviews.map((r) => {
                                        ...
                                        const moderator = r.moderated_by
                                            ? [r.moderated_by.first_name, r.moderated_by.last_name].filter(Boolean).join(" ")
                                            : r.moderated_by_id
                                                ...
                                                <TableCell align="right">
                                                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                                                        <IconButton
                                                            size="small"
                                                            title="Одобрить"
                                                            onClick={() => updateSiteReviewStatus(r, "APPROVED")}
                                                        >
                                                            <FiCheck />
                                                        </IconButton>
                                                        <IconButton
                                                            size="small"
                                                            title="Отклонить"
                                                            onClick={() => updateSiteReviewStatus(r, "REJECTED")}
                                                        >
                                                            <FiX />
                                                        </IconButton>
                                                        <IconButton
