...
export default function AdminLayout() {
    ...
    const groupedNav = React.useMemo(() => {
        ...
        const overview = overviewItem && filterItem(overviewItem) ? overviewItem : null;

        const groups = [
            {
                label: "Пользователи",
                items: NAV_ITEMS.filter((i) =>
                    ["/admin/users", "/admin/groups-roles", "/admin/black-list"].includes(i.to)
                ).filter(filterItem),
            },
            {
                label: "Ресурсы",
            ...
            <List sx={{ flexGrow: 1, py: 1, overflowY: "auto" }}>
                {groupedNav.overview && (
                    ...
                    >
                        ...
                        <ListItemText primary={groupedNav.overview.label} primaryTypographyProps={{ fontSize: 14, fontWeight: 600 }} />
                    </ListItemButton>
                )}
                ...
                {groupedNav.groups.map((group) => (
                    <Box key={group.label} sx={{ mt: 2 }}>
                        <Typography
                            ...
                            sx={{ px: 3, py: 0.5, display: "block", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}
                        >
                            {group.label}
                        </Typography>
                        <List disablePadding>
        ...
        <Box sx={{ display: "flex", minHeight: "100dvh", bgcolor: "#F5F5F5" }}>
            ...
            >
                <Toolbar sx={{ px: { xs: 2, md: 3 }, height: "64px" }}>
                    ...
                    {isDesktop && (
                        <Stack direction="row" spacing={0.5} sx={{ ml: 2, height: "100%", alignItems: "center" }}>
                            {groupedNav.overview && (
                                <Button
                                    component={NavLink}
                                    to={groupedNav.overview.to}
                                ...
                                return (
                                    ...
                                    >
                                        <Button
                                            sx={{
                                                ...
                                                borderRadius: 1.5,
                                                "&:hover": {
                                                    bgcolor: isGroupActive ? "action.selected" : "action.hover",
                                                }
                                            }}
                                        >
                                            {group.label}
                                    ...
                                    >
                                        {group.items.map((item) => (
                                            ...
                                            >
                                                <ListItemIcon sx={{ minWidth: 32, fontSize: 16, color: "text.secondary" }}>
                                                    {item.icon}
                                                </ListItemIcon>
                                                <ListItemText
                                                    primary={item.label}
                    ...
                    {isDesktop && (
                        <Stack direction="row" spacing={0.75} alignItems="center" sx={{ mr: 2 }}>
                            {accessLoading ? (
                                <CircularProgress size={16} />
                            ) : access?.isRoot ? (
                                <Chip size="small" label="ROOT" color="primary" variant="outlined" />
                            ) : activeGroups.length ? (
                                activeGroups.slice(0, 3).map((g: any) => (
                                    <Chip key={g.id} size="small" label={g.code || g.name} variant="outlined" />
                                ))
                            ) : null}
                        </Stack>
                    )}
